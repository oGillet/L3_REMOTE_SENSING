...
`r emo::ji("question")` Vous devez calculer la superficie des espaces verts au cours de l'année 2017 et les 8 communes de la zone d'étude.
...
```{r , echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE}
df <- data.frame(Date=c(rep(as.Date(c("10/14/2017"),"%m/%d/%y"),8)),  
                 Superficie=c(1.280,2.266,1.683,2.79,5.756,0.199,4.888,4.074),
                 Commune=c("Bohars","Brest","Gouesnou","Guilers","Guipavas","Le Relecq-Kerhuon","Plougastel-Daoulas","Plouzané"))

ggplot2::ggplot(df, ggplot2::aes(x=Commune, y=Superficie, fill=Commune)) +
  ggplot2::geom_bar(stat="identity")+
  ggplot2::labs(title = "Superficie des espaces végétalisés par communes",subtitle = "10 octobre 2017", x="Commune", y="Superficie (km2)")+ 
